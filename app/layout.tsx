...
export default function RootLayout({ children }: { children: React.ReactNode }) {
  useEffect(() => {
    // Dynamic import Three.js dari CDN
    const initThree = async () => {
      ...
      fontLoader.load(
        ...
        (font) => {
          ...
          // Dekorasi spheres, octahedrons, torus, shards, particles ...
          // (Salin semua kode dekorasi dari versi sebelumnya, tidak perlu diubah)
          // Karena panjang, saya sertakan ringkasannya di bawah.
          // -------------------------------
      ...
      function updateCountdownMesh() {
        if (!countdownMesh || !fontLoader) return;
        ...
        const newGeometry = centerGeometry(
          new TextGeometry(currentString, {
            font: fontLoader, // Perlu simpan font
            size: 7.5,
            height: 3.5,
            curveSegments: 14,
            bevelEnabled: true,
      ...
      // Simpan font agar bisa dipakai update (perlu variabel global di scope ini)
      let loadedFont = null;
      // Override fontLoader.load untuk menyimpan font
      const originalLoad = fontLoader.load;
      fontLoader.load = function(url, onLoad, onProgress, onError) {
        originalLoad.call(this, url, (font) => {
          loadedFont = font;
          onLoad(font);
        }, onProgress, onError);
      };
    ...
    // Cleanup (opsional)
    return () => {
      // Hentikan animasi, hapus renderer, dll.
