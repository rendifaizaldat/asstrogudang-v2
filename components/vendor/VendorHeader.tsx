...
export default function VendorHeader({ total }: { total: number }) {
  const [showAdd, setShowAdd] = useState(false);

  return (
    <div className="flex justify-between items-end">
      <div>
        <h2 className="text-2xl font-bold text-slate-800">Manajemen Vendor</h2>
      ...
      >
        <span className="text-xl">+</span> Tambah Vendor
      </button>

      <VendorModal isOpen={showAdd} onClose={() => setShowAdd(false)} />
    </div>
  );
}
